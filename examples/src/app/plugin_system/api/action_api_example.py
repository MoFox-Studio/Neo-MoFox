...
async def main() -> None:
    ...
    actions_for_chat = await action_api.filter_actions(
        list(actions.values()),
        chat_type=ChatType.PRIVATE,
    )
    print(f"私聊可用 Action 数量: {len(actions_for_chat)}")

    schemas_for_chat = action_api.get_action_schemas(actions_for_chat)
    print(f"私聊 Action schema 数量: {len(schemas_for_chat)}")

    stream_id = "demo_action_stream"
    available_actions = await action_api.filter_actions(
        list(actions.values()),
        stream_id=stream_id,
    )
