...
logger = log_api.get_logger("neo_booku_memory.command_tool")


def _service(plugin: Any) -> NeoMemoryService:
    return NeoMemoryService(plugin=plugin)
...
def _pick_tag_triplet(
    options: dict[str, list[str]], *, required: bool = False,
) -> tuple[list[str], list[str], list[str]]:
    tag_option_keys = (
        "core_tags", "core", "diffusion_tags", "diffusion",
        "opposing_tags", "opposing", "triple_tags", "triplet",
    )
...
class NeoBookuMemoryCommandTool(BaseTool):
    """统一命令工具：memory_command。"""

    name: str = "memory_command"
    description: str = (
        "Booku Memory 命令工具。支持 help/search/read/create/update/delete。"
        "使用该工具应当非常频繁，多记多读。"
    )

    async def execute(
        self, command: Annotated[str, "Booku Memory 命令字符串。支持 help/search/read/create/update/delete 和 && 串联；不确定格式时先执行 help。"]
    ) -> tuple[bool, str | dict]:
        raw_command = (command or "").strip()
        if not raw_command:
            return True, self._build_result(ok=False, executions=[{"command": "", "success": False, "error": "command 不能为空"}])

        segments = _split_segments(raw_command)
        if not segments:
            return True, self._build_result(ok=False, executions=[{"command": raw_command, "success": False, "error": "未解析到可执行命令"}])

        executions: list[dict[str, Any]] = []
        service: NeoMemoryService | None = None
        ...
        for segment in segments:
            try:
                ...
                return True, self._build_result(ok=False, executions=executions)

        return True, self._build_result(ok=True, executions=executions)

    @staticmethod
    def _build_result(*, ok: bool, executions: list[dict[str, Any]]) -> dict[str, Any]:
        return {"action": "memory_command", "ok": ok, "executed": len(executions), "results": executions}

    async def _execute_single(
        self, *, service: NeoMemoryService | None, operation: str, options: dict[str, list[str]],
    ) -> dict[str, Any]:
        if operation == "help":
            return {"action": "help", "content": BOOKU_MEMORY_COMMAND_MANUAL}
        ...
        if operation == "search":
            ...
            return await service.search_memory_entries(
                top_n=top_n, query_text=_pick_option(options, "query", "q"),
        ...
        if operation == "create":
            ...
            core_tags, diffusion_tags, opposing_tags = _pick_tag_triplet(options, required=True)
            bucket = "archived" if status in {"archived", "expired"} else "emergent"
            return await service.create_memory(
                title=title, content=content, bucket=bucket,
                core_tags=core_tags, diffusion_tags=diffusion_tags, opposing_tags=opposing_tags,
        ...
        if operation == "update":
            ...
            core_tags, diffusion_tags, opposing_tags = _pick_tag_triplet(options)
            return await service.update_memory_by_id(
                memory_id=memory_id,
                title=_pick_option(options, "title"),
                content=_pick_option(options, "content", "body"),
                core_tags=core_tags or None, diffusion_tags=diffusion_tags or None,
                ...
                event_start_at=_parse_float(_pick_option(options, "event_start_at", "start_at"), 0.0)
                if _pick_option(options, "event_start_at", "start_at") is not None else None,
                event_end_at=_parse_float(_pick_option(options, "event_end_at", "end_at"), 0.0)
                if _pick_option(options, "event_end_at", "end_at") is not None else None,
        ...
        if operation == "delete":
            ...
            return await service.delete_memories(memory_ids=ids, hard=_parse_bool(_pick_option(options, "hard"), False))

        raise ValueError(f"不支持的命令: {operation}，可先执行 help 查看支持的命令与参数说明")


class NeoBookuTemporaryMemoTool(BaseTool):
    """短期临时备忘录工具。"""
    ...
    async def execute(
        self,
        content: Annotated[str, "要记录的短期关键内容。注意这不是长期记忆，只用于临时便签提醒。"],
        expire_hours: Annotated[float, "相对过期时间，单位小时，默认 2.0。到期后会自动失效。"] = 2.0,
    ) -> tuple[bool, str | dict]:
        service = _service(self.plugin)
        try:
            result = await service.create_temporary_memo(
                content=content, expire_hours=expire_hours, stream_id=self.get_current_stream_id(),
            )
        except Exception as error:  # noqa: BLE001
            logger.error(f"temporary_memo 执行失败: {error}", exc_info=True)
            return True, {"action": "create_temporary_memo", "ok": False, "error": str(error)}
        return True, {"action": "create_temporary_memo", "ok": True, "result": result}
