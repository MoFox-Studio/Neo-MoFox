...
class TestMediaManagerRecognizeMedia:
    ...
    async def test_store_sent_media_can_be_read_by_id(
        self, make_manager: Any, tmp_path: Path, media_type: str
    ) -> None:
        """四种已发送媒体落盘入库后，凭 ID 可以读回原始字节。"""
        manager = make_manager()
        config = manager._config
        config.pending_folder = tmp_path / "pending"
        config.pending_folder.mkdir()
        for attr in ("images_folder", "emojis_folder", "voices_folder", "videos_folder"):
            folder = tmp_path / attr
            folder.mkdir()
            setattr(config, attr, folder)

        data = "base64|aGVsbG8="
        media_hash = manager.compute_media_hash(data)
        with (
            patch.object(manager._repository, "save_recognized_media", new_callable=AsyncMock) as save,
            patch.object(manager._repository, "get_media_info", new_callable=AsyncMock) as image_info,
            patch.object(manager._repository, "get_voice_info", new_callable=AsyncMock) as voice_info,
            patch.object(manager._repository, "get_video_info", new_callable=AsyncMock) as video_info,
        ):
            folder = manager._file_store.category_folder_for(media_type)
            extension = {"image": ".jpg", "emoji": ".png", "voice": ".wav", "video": ".mp4"}[media_type]
            expected_path = folder / f"{media_hash[:16]}_{media_type}{extension}"
            image_info.return_value = None
            voice_info.return_value = None
            video_info.return_value = None
            info_mock = {"voice": voice_info, "video": video_info}.get(media_type, image_info)
            info_mock.return_value = {"path": str(expected_path)}

            assert await manager.store_media(data, media_type) is True
            assert expected_path.read_bytes() == b"hello"
            assert await manager.get_media_file(media_hash) == "aGVsbG8="
            save.assert_awaited_once_with(
                media_hash, media_type, str(expected_path), description=None, processed=False
            )

    @pytest.mark.asyncio
    async def test_store_sent_media_rejects_missing_file(self, make_manager: Any, tmp_path: Path) -> None:
        """落盘失败不能留下虚假的媒体记录。"""
        manager = make_manager()
        with (
            patch.object(manager._file_store, "save_to_pending", new_callable=AsyncMock, return_value=tmp_path / "missing.jpg"),
            patch.object(manager._repository, "save_recognized_media", new_callable=AsyncMock) as save,
        ):
            assert await manager.store_media("base64|aGVsbG8=", "image") is False
